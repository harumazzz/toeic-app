...
class ExamScreen extends HookConsumerWidget {
  ...
  Widget build(final BuildContext context, final WidgetRef ref) {
    ...
    ref
      ...
      ..listen<SubmissionState>(
        submissionNotifierProvider,
        (final previous, final next) {
          switch (next) {
            case SubmissionLoading():
              // Show loading indicator if needed
              break;
            case SubmissionSuccess(:final result):
              ToastService.success(
                context: context,
                message: context.t.exam.submission.successMessage,
              );
              ...
              break;
            case SubmissionError(:final message):
              ToastService.error(
                context: context,
                message: message,
              );
    ...
    return Scaffold(
        ...
        ExamError() => Center(
          child: Column(
            ...
            children: [
              ...
              ElevatedButton(
                onPressed: () =>
                    ref.read(examNotifierProvider.notifier).loadExam(examId),
      ...
      floatingActionButton: examState is ExamLoaded
          ? Consumer(
        ...
        actions: [
          ...
          ElevatedButton(
            onPressed: () async {
              ...
              // Convert Map<int, String> to List<SubmitAnswer>
              final answers = userAnswers.entries
                  .map(
                    (final entry) => SubmitAnswer(
              ...
              // Get attemptId from attempt state
              final attemptState = ref.read(attemptNotifierProvider);
              if (attemptState is AttemptStarted) {
                await ref
                    .read(submissionNotifierProvider.notifier)
                    .submitAnswers(
                      attemptId: attemptState.attempt.attemptId,
                      answers: answers,
                    );
              }
            },
